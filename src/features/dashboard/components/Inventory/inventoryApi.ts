...
export async function fetchAllProductsApi({
  ...
  try {
    ...
    const response = await axiosInstance.get<{ data: FetchProductsResponse }>(
      `/products?page=${page}&limit=${limit}&sort=${sort}&order=${order}&search=${search}&isDeleted=${isDeleted}&category=${category}&status=${status}&organizationId=${orgId}`
    );
...
export async function getAllCategories(): Promise<string[]> {
  try {
    const orgId = getOrgId();
    const response = await axiosInstance.get<{ data: string[] }>(`/products/categories?organizationId=${orgId}`);
    return response.data?.data;
...
export async function getAllSuppliers(): Promise<string[]> {
  try {
    const orgId = getOrgId();
    const response = await axiosInstance.get<{ data: string[] }>(`/products/suppliers?organizationId=${orgId}`);
    return response.data?.data;
