...
const EditProductCard = () => {
  ...
  const [images, setImages] = useState(formData.photos ||[defaultImage]);  //It holds Image links links for SLider Image Preview
  const [imagesFiles, setImagesFiles] = useState([defaultImage]); //It holds image files which are required while uploading
  ...
  const handleSave = () => {
    if (product.price?.[0]?.price !== formData.price) {
      dispatch(updatePrice({ productId: product._id, newPrice: formData.price }));
    }
